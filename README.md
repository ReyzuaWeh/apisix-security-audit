...
## 25th August 2025
Continue reviewing plugins. The main focus are : Observability Plugin and Serverless Plugins.
...
Event thought the Prometheus plugin is pre-installed, you need to enable it in the `config.yaml` file. Add the following configuration under the `plugins` section:
```yaml
plugins:
  - name: prometheus
```
...
The dafault log format only use route configuration
```bash
curl "http://127.0.0.1:9180/apisix/admin/routes" -X PUT \
  -H "X-API-KEY: ${admin_key}" \
  -d '{
...
You may create a condition wheter a requests go to log or not. Here's the example
```bash
curl "http://127.0.0.1:9180/apisix/admin/routes" -X PUT \
  -H "X-API-KEY: ${admin_key}" \
  -d '{
...
##### Test
As for test, ypu can see the result of the configuration in *Kafka*'s log. If you use `docker-compose.yml` configuration, you may use this command
...
The result will definately shows the log wuth table. If you haven't access the route, then you will not see any value
